...
int inv1(int i) {if (i == 1) return 1; return (mod - ((mod / i) * inv1(mod % i)) % mod) % mod;}
...
for(int i = 0; i < n; i++) {
    ...
    if(ans > v[n-1]) {
        ...
        auto it = upper_bound(v.begin(), v.end(), ans) - v.begin();
        it--;
        if(it >= i + m - 1) {
            int kp = it - i;
            res += (nCr(kp, (m - 1)));
