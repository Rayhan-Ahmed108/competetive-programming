...
map<char, int> m;
for(char c : s) m[c]++;
int countB = m['B'], countS = m['S'], countC = m['C'];
int64_t lo = 0, hi = 1e13, res = 0;
while(lo <= hi) {
    int64_t mid = lo + (hi - lo) / 2;
    lll bB = max((lll)0, (lll)mid * countB - v[0]);
    lll bS = max((lll)0, (lll)mid * countS - v[1]);
    lll bC = max((lll)0, (lll)mid * countC - v[2]);

    lll need = bB*v[3] + bS*v[4] + bC*v[5];
